...
def get_house_infor(text):
    ...
    mianji=total_dic.get('建筑面积：')
    print(mianji)
    niandai=total_dic.get('建造年代：')
    mianjilist=[]
    niandailist=[]
    
    for i in mianji:
        newmianji=i.strip('平方米')
        mianjilist.append(newmianji)
    print(mianjilist)
    for j in niandai:
        newniandai=j.strip('年')
        niandailist.append(newniandai)
    print(niandailist)
    
    return(niandailist,mianjilist)
...
def main():
    ...
    for i in range (1,2):
        ...
        niandailist=return_data[0]
        csv_list3.append(niandailist)
        mianjilist=return_data[1]
        csv_list4.append(mianjilist)

    df1=pd.DataFrame(csv_list3)
    df1.to_csv('test3.csv')
    df2=pd.DataFrame(csv_list4)
...
mianji_totallist=[]
zidian=[]
f1=open('test3.csv','r',encoding='utf-8')
csvreader1=csv.reader(f1)
for niandai in csvreader1:
    niandai_totallist.append(niandai[1:])
#print(mingcheng_totallist[1])
    
f2=open('test4.csv','r',encoding='utf-8')
csvreader2=csv.reader(f2)
for mianji in csvreader2:
    mianji_totallist.append(mianji[1:])
print(len(mianji_totallist[1]))

key=niandai_totallist[1]
value=mianji_totallist[1]
...
print(zidian)
paixu=sorted(zidian,key=lambda x:x[0],reverse=False)
print(paixu)
...
c = (
        ...
        .add_yaxis("面积", finaly)
        .set_global_opts(
            title_opts=opts.TitleOpts(title="年份与面积"),
            xaxis_opts=opts.AxisOpts(splitline_opts=opts.SplitLineOpts(is_show=True),min_=1982,max_=2018,type_='value',max_interval=5),
            yaxis_opts=opts.AxisOpts(splitline_opts=opts.SplitLineOpts(is_show=True)),
        )
    )
c.render()
#make_snapshot(snapshot, c.render(), "nianfen.png")
